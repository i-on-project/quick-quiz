...
class InSessionController(private val answerService: AnswersService,
                          private val sessionService: SessionService,
                          private val quizService: QuizService) : UnauthMainController() {
    ...
    fun joinSession(@RequestBody input: JoinSessionInputModel): ResponseEntity<Any> {
        val ansDoc = sessionService.joinSession(input)
    ...
    fun giveAnswer(request: HttpServletRequest, @RequestBody input: GiveAnswerInputModel): ResponseEntity<Any> {
        return ResponseEntity.ok().body(answerService.giveAnswer(input))
    }
    ...
    fun getAnswer(@PathVariable answerId: String, request: HttpServletRequest): ResponseEntity<Any> {
        val ansDoc = answerService.getAnswer(answerId)
        if(!sessionService.checkSessionIsLive(ansDoc.sessionId)) {
    ...
    fun getAllQuizzesForAnswerSession(@PathVariable answerId: String): ResponseEntity<Any> {
        val ansDoc = answerService.getAnswer(answerId) //TODO: checks here
        val quizzes = quizService.getAllSessionAnswersQuizzes(ansDoc.sessionId)
